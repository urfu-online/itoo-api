...
def profile_detail(request):
    ...
    if request.method == "GET":
        ...
        profile_params = {
            'contract_number': 3,
            'client_name': "{first_name} {last_name} {second_name}".format(
                first_name=profile.values_list('first_name', flat=True).get(pk=1),
                last_name=profile.values_list('last_name', flat=True).get(pk=1),
                second_name=profile.values_list('second_name', flat=True).get(pk=1),
            ),
            'client_phone': profile.values_list('phone', flat=True).get(pk=1),
            'client_email': request.user.email,
            'amount': '2000'
        }
        return redirect_params('https://ubu.urfu.ru/pay/', profile_params)
